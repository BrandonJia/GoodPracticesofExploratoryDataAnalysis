...
```{r}
pivot_data %>% ggplot(aes(x = CohortIndex, y = CohortMonth, fill = count)) + geom_tile() + ggtitle(label = 'The Retention Rate Every Month' ) +
  xlab('Cohort Index') +
  ylab('Cohort Month')
```
...
pivot_data %>% ggplot(aes(x = CohortIndex, y = CohortMonth, fill = Mean)) + geom_tile() + ggtitle(label = 'The Quantity of Products Per Transaction On Average' ) +
  xlab('Cohort Index') +
  ylab('Cohort Month')
```
